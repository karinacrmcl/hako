...
export default function UserInfo({
  ...
  return (
    <div className="bg-white w-userinfo rounded-lg shadow-xl py-6 flex px-4 justify-between flex flex-col mt-12 mx-auto">
      <div className="flex items-center">
        {user.username && avatarUrl ? (
          <img
            className="rounded-full avatar-userinfo  object-cover flex"
            alt={user.username}
            src={avatarUrl?.basic}
          />
        ) : (
      ...
      <div className="container flex mt-4 justify-evenly w-4/5 select-none">
        {followers == undefined || following == undefined ? (
          <Skeleton count={1} width={677} height={24} />
        ) : (
          <>
            <p className="flex font-medium flex-col text-sm items-center ">
              <span className="font-semibold text-xl">{photosCount}</span>{" "}
              publications
            </p>
            <div
              className=" flex font-medium flex-col text-sm items-center "
              onMouseEnter={() =>
                followerCount == 0 ? null : setIsFollowersHovered(true)
              }
              onMouseLeave={() => setIsFollowersHovered(false)}
            >
              <span className="font-semibold text-xl">{followerCount}</span>
            ...
            >
              <span className="font-semibold text-xl">{following.length}</span>
